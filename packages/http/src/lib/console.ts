...
function consoleLogger(method: StyledConsoleArgs['method'], ...args: unknown[]) {
  if (process.env.NODE_ENV === 'production' && !process.env.ENVIRONMENT) return;
...
export const clientLogger = ({ status, reqData, resData, method: consoleMethod = 'log' }: ApiLoggerArgs) => {
