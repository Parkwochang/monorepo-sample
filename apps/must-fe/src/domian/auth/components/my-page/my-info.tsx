...
import { MyPageLoading } from '@/shared/components/box';
...
export const MyInfo = ({ memberId }: { memberId: number }) => {
  const { data: member, isFetching } = useGetMemberQuery(memberId);
  const { push } = useRouter();

  return (
    <DeferredLayout loading={isFetching} fallback={<MyPageLoading />}>
      <div
        className="flex-center justify-between gap-3 px-3 py-3 bg-white rounded-lg mx-3 shadow-xs active:bg-gray-100 active:scale-[0.98] transition-all duration-200"
        onClick={() => push(PATH.my.profile)}
      >
