...
export async function getTokenExpiredTime(token: AuthEntity.UserInfo) {
  ...
  const now = getTime(new Date()); // 한국 시간 변환

  // console.log('token exp : ', new Date(exp * 1000), exp * 1000);
  // console.log('node now : ', new Date(now), now);
  // console.log('kr now : ', new Date(now + 9 * 60 * 60 * 1000), addHours(new Date(), 9));

  return (exp - 15) * 1000 < now;
}
...
export async function validateUserPermissions(role: AuthEntity.UserInfo['role'][]) {
  ...
  if (userPermissions && role.includes(userPermissions)) {
